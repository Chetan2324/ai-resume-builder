...
function App() {
  ...
  const handleLoad = (idToLoad) => {const id = idToLoad || loadId; if (!id) return toast.warn('Please enter or select a Resume ID to load.'); axios.get(`${process.env.REACT_APP_API_URL}/resumes/${id}`).then(res => {if (res.data) {setResumeData(res.data); setCurrentResumeId(res.data._id); setLoadId(res.data._id); toast.success('Resume loaded successfully!');} else {toast.error('No resume found with that ID.');}}).catch(err => { console.error(err); toast.error('Error loading resume.'); });};
  const handleSave = () => {if (currentResumeId) {axios.put(`${process.env.REACT_APP_API_URL}/resumes/update/${currentResumeId}`, resumeData).then(res => toast.success('Resume updated successfully!')).catch(err => { console.error(err); toast.error('Error updating resume.'); });} else {axios.post(`${process.env.REACT_APP_API_URL}/resumes/add`, resumeData).then(res => {const newId = res.data.id; toast.success(`Resume saved! New ID: ${newId}`); setCurrentResumeId(newId); setLoadId(newId); addIdToStorage(newId);}).catch(err => { console.error(err); toast.error('Error saving resume.'); });}};
  ...
  return (
    // The outer div is now just for the background color
    <div className="min-h-screen bg-gray-200 font-sans py-8">
      ...
      {/* NEW: A centered container for all the content */}
      <div className="container mx-auto px-4">
        <header className="flex justify-between items-center mb-8"><h1 className="text-4xl font-bold text-gray-800">Smart Resume Builder</h1><div className="flex gap-4 items-center"><button onClick={handleLoadTemplate} className="bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-6 rounded-lg shadow-lg" title="Load example data">Load Template</button><button onClick={handleClearForm} className="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-6 rounded-lg shadow-lg" title="Clear all fields">Clear Form</button><button onClick={handleSave} className="bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-6 rounded-lg shadow-lg">{currentResumeId ? 'Update Resume' : 'Save as New'}</button><button onClick={handlePdfExport} className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-6 rounded-lg shadow-lg">Download PDF</button></div></header>
